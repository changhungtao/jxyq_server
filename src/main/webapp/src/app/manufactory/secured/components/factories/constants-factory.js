 ...
 * @constructor
 */
jxmnfsec.constants.Factory.$inject = [
  '$rootScope',
  '$http', 
  '$q'
];
jxmnfsec.constants.Factory = function($rootScope,$http, $q) {
  ...
  function gotGENDERS(){
   ...
   //终端型号模板类型
  var TEMPLATETYPES = [];
  var TEMPLATETYPESPromise = getTemplateTypes().then(
    function(data){
  ...
  function getTemplateTypes(){
    var req = {
      method: 'GET',
      url: '/common/constants/template_types'
    }
    return getConstants(req);
  };
  ...
  var service = {
    ...
    gotTEMPLATETYPES:gotTEMPLATETYPES,
    TEMPLATETYPESPromise:TEMPLATETYPESPromise
  };
