...
jxmnfsec.main.user_center.password.Ctrl.$inject = [
    'basic',
    'md5',
    '$timeout'
];
...
jxmnfsec.main.user_center.password.Ctrl= function($scope,basic,md5) {
  ...
  this.label = 'some other label from main.user_center.password controller';
    ...
    $scope.saveNewPwd = function(){
        console.log('test');
        basic.change_password({old_password:md5.createHash($scope.chPassword.password),
                               new_password:md5.createHash($scope.chPassword.new_password)}).then(function(res){
